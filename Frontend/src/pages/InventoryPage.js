...
const InventoryPage = () => {
  ...
  const fetchBooks = async () => {
    try {
      const { data } = await axios.get(`${process.env.REACT_APP_API_URL}/api/inventory`);
      setBooks(data);
    } catch (error) {
  ...
  const fetchLowStockBooks = async () => {
    try {
      const { data } = await axios.get(`${process.env.REACT_APP_API_URL}/api/inventory/low-stock`);
